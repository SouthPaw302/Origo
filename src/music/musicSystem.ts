...
export interface PreparedMusicExport {
  blob: Blob;
  filename: string;
  label: 'WAV' | 'MIDI' | 'Session' | 'Aether';
}
...
export class OrigoMusicSystem {
  ...
  public start(engine: SimulationEngine) {
    ...
    this.clock = new OrigoMusicClock(
      ...
      world.clock.beatsPerBar,
      world.clock.subdivisionsPerBeat,
      engine.stepCount
    );
    this.director.reset();
    ...
    this.session = {
      ...
      musicWorldId: world.id,
      musicWorldName: world.name,
      tempoBpm: tempo,
  ...
  public prepareSessionJson(): PreparedMusicExport | null {
    ...
    return {
      blob,
      filename: `${safeName(this.session.presetName)}-${this.session.id}.origo.json`,
      label: 'Session',
    };
  }

  public prepareAetherManifest(): PreparedMusicExport | null {
    if (!this.session) return null;
    this.refreshAnalysis(true);
    this.session.sections = this.sectionTracker.getSections();
    ...
    return {
      blob,
      filename: `${safeName(this.session.presetName)}-${this.session.id}.aether.json`,
      label: 'Aether',
    };
  }

  public prepareMidi(): PreparedMusicExport | null {
    if (!this.session || this.session.events.length === 0) return null;
    const midi = sessionToMidi(this.session);
    return {
      blob: new Blob([midi], { type: 'audio/midi' }),
      filename: `${safeName(this.session.presetName)}-${this.session.id}.mid`,
      label: 'MIDI',
    };
  }
  ...
  // Compatibility helpers for integrations that still expect an immediate browser download.
  public exportSessionJson() {
    const artifact = this.prepareSessionJson();
    if (artifact) downloadBlob(artifact.blob, artifact.filename);
  }

  public exportAetherManifest() {
    const artifact = this.prepareAetherManifest();
    if (artifact) downloadBlob(artifact.blob, artifact.filename);
  }

  public exportMidi() {
    const artifact = this.prepareMidi();
    if (artifact) downloadBlob(artifact.blob, artifact.filename);
  }

  public async exportWav() {
    const artifact = await this.prepareWav();
    if (artifact) downloadBlob(artifact.blob, artifact.filename);
  }
